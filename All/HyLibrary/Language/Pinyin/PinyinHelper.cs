namespace HyLibrary.Language.Pinyin
{
    using HyLibrary.Language.Pinyin.Internal;
    using System;
    ...
    public class PinyinHelper
    {
        ...
        public static string GetChineseText(string pinyin)
        {
            ...
            if (PyCode.hashcodes.ContainsKey(key))
            {
                return PyCode.hashcodes[key];
            }
        ...
        /// <param name="dest">目标编码</param>
        /// <returns>目标编码文本</returns>
        public static string ConvertEncoding(string text, Encoding source, Encoding dest)
        {
            var srcBytes = source.GetBytes(text);
            var dstBytes = Encoding.Convert(source, dest, srcBytes);
            return dest.GetString(dstBytes);
        }
